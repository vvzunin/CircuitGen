...
int main(int argc, char** argv) {
    ...
    while ((opt = getopt_long(argc, argv, short_opts, long_opts, nullptr)) !=
        ...
        case 'j':
            jsonPath = optarg;
            LOG(INFO) << "JSON path set to: " << jsonPath;
            break;
        case 'f':
            filePath = optarg;
        ...
        case 'd':
            ...
            LOG(INFO) << "Directory path set to: " << directoryPath;
            break;
        case 't':
            sub = atoi(optarg);
            if (sub <= 0) {
                LOG(ERROR) << "Threads number must be more than 0!";
                return 1;
            }
            if (sub > 255) {
                LOG(ERROR) << "Threads number must be less (or equal) than 255!";
                return 1;
            }
            threadsNumber = sub;
            LOG(INFO) << "Threads number set to: " << static_cast<int>(threadsNumber);
            break;
        case 'b':
            makeBalanced = true;
            LOG(INFO) << "Make balanced option enabled";
            break;
        case 'r':
            makeResyn2 = true;
            LOG(INFO) << "Make resyn2 option enabled";
            break;
        case 'm':
            moveToFolder = true;
            LOG(INFO) << "Move to folder option enabled";
            break;
        case 'B':
            toBench = true;
            LOG(INFO) << "To bench option enabled";
            break;
        case 'F':
            toFirrtl = true;
            LOG(INFO) << "To Firrtl option enabled";
            break;
        case 'c':
            calcStats = true;
            LOG(INFO) << "Calculate stats option enabled";
            break;
        case 'h':
            std::cout << "---------------------------------------------------"
                << std::endl;
        ...
        case 'l':
            libName = optarg;
            LOG(INFO) << "Library name set to: " << libName;
            break;
        case '?':
            // Unknown option or missing argument
            LOG(WARNING) << "Unknown option or missing argument detected";
            break;
        default:
            LOG(ERROR) << "Unknown error while parsing command line arguments.";
            return 1;
        }
    }
    ...
    if (!jsonPath.empty() && !filePath.empty() && !directoryPath.empty()) {
        LOG(ERROR) << "Not allowed to use at the same time path to verilog file and to json.";
        return 1;
    }
    ...
    auto runInMultithread = [&](std::string path, std::string graph) {
        LOG(DEBUG) << "Running in multithread for path: " << path << ", graph: " << graph;
    ...
    if (jsonPath.size()) {
        ...
        if (!(makeResyn2 || makeBalanced || toBench || toFirrtl || calcStats)) {
            LOG(INFO) << "No tasks to execute, exiting.";
            return 0;
        }
        ...
        for (auto [main_path, allGraphs] : allRes) {
            for (auto graph : allGraphs) {
                ...
                LOG(DEBUG) << "Processing graph from JSON: path: " << path << ", graph: " << graph;
    ...
    // used for parsing one file or dir
        ...
        if (filePath.size()) {
            ...
            LOG(DEBUG) << "Processing single file: path: " << path << ", graph: " << graph;
            runInMultithread(path, graph);
        }
            ...
            for (const auto& dirEntry : recursive_directory_iterator(directoryPath)) {
                if (!dirEntry.is_directory()) {
                    ...
                    if (file.extension() == ".v") {
                        if (moveToFolder) {
                            LOG(DEBUG) << "Moving file to new folder: " << file.parent_path() / file.stem();
                            std::filesystem::create_directory(file.parent_path() /
                                file.stem());

                            filePath = file.parent_path() / file.stem() / file.filename();
                            LOG(DEBUG) << "Renaming file from: " << file << " to " << filePath;
                            std::filesystem::rename(file, filePath);
                        }
                        LOG(DEBUG) << "Processing verilog file: path: " << filePath.parent_path() << ", graph: " << filePath.stem();
    ...
    std::clog << "Time taken: " << duration.count() << " microseconds"
        << std::endl;
    LOG(INFO) << "Total execution time: " << duration.count() << " microseconds";
    return 0;
}